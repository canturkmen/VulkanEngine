...
namespace std {
	template<>
	struct hash<VulkanEngine::VulkanModal::Vertex>
	{
		size_t operator()(VulkanEngine::VulkanModal::Vertex const& vertex) const
		{
			size_t seed = 0;
...
namespace VulkanEngine {
	...
	std::unique_ptr<VulkanModal> VulkanModal::CreateModelFromFile(VulkanDevice& device, const std::string& filepath)
	{
		...
		return std::make_unique<VulkanModal>(device, builder);
	}
	...
	void VulkanModal::Builder::LoadModel(const std::string& filepath)
	{
